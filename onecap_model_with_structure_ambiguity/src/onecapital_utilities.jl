...
mutable struct PolicyFunctions
  ...
  hk_F::Array{Float64,1}
  hz_F::Array{Float64,1}

  hk_B::Array{Float64,1}
  hz_B::Array{Float64,1}

end

function dstar_onecapital_quadratic!(d::Array{Float64,1},
                            zz::Array{Float64,1},
                            II::Int64,
                            rho::Float64,
                            V::Array{Float64, 1},
                            model::OneCapitalEconomy{Float64},
                            preloadd::Array{Float64, 1},
                            C::Array{Float64, 1},
                            foc::Int64,
                            clowerlim::Float64)
    ...
    for i=1:II
        ...
        if rho == 1.0
            if foc == 0
                ...
                d_root_1 = (-b + sqrt(b^2 - 4*a*c))/(2*a)
                fun_val_1 = delta*log(alpha - d_root_1) +d_root_1 - phi*d_root_1^2/2
                d_root_2 = (-b - sqrt(b^2 - 4*a*c))/(2*a)
                fun_val_2 = delta*log(alpha - d_root_2) +d_root_2 - phi*d_root_2^2/2
                if fun_val_1 >= fun_val_2
                    d[i] = d_root_1;
                else
                    d[i] = d_root_2;
                end
            ...
            elseif foc == 1
                ...
                c_root_1 = max((-b + sqrt(b^2 - 4*a*c))/(2*a),clowerlim)
                fun_val_1 = delta*log(c_root_1) +(alpha - c_root_1) - phi*(alpha - c_root_1)^2/2
                c_root_2 = max((-b - sqrt(b^2 - 4*a*c))/(2*a),clowerlim)
                fun_val_2 = delta*log(c_root_2) +(alpha - c_root_2) - phi*(alpha - c_root_2)^2/2
                if fun_val_1 >= fun_val_2
                    C[i] = c_root_1;
                    d[i] = alpha - c_root_1;
                else
...
function dstar_onecapital_log!(d::Array{Float64,1},
                            zz::Array{Float64,1},
                            II::Int64,
                            rho::Float64,
                            V::Array{Float64, 1},
...
function robust_control_variables!(s1::Array{Float64,1},
    ...
    for i = 1:II
        ...
        # if  z == zbar
            ...
            # println("i: ", i, ", z:", z, ", lambda_1: ", lambda_1, ", lambda_2: ", lambda_2, ", vz: ", vz, ", function_1: ", A1*s1_1 + A2*s2_1, ", function_2: ", A1*s1_2 + A2*s2_2, ", s1_1: ", s1_1, ", s2_1: ", s2_1, ", s1_2: ", s1_2, ", s2_2: ", s2_2)
            ...
            elseif (A1*s1_1 + A2*s2_1 >= A1*s1_2 + A2*s2_2) && (lambda_2 > 0)
                ...
                SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSTOP
...
function drifts!(mu_1::Array{Float64, 1},
    ...
    for i=1:II
        ...
        if logadjustment == true
            mu_k =  log(1+d[i]*phi)/phi + beta*z - eta - dot(s_k,s_k)/2;
        else
            mu_k = d[i] - (phi*d[i]^2)/2 + (beta*z - eta - dot(s_k,s_k)/2);
        end
        mu_z[i] = -betaz*(z-zbar);
...
function create_Aval_reflective!(Aval, d_, c_, e_, II)
...
function value_function_onecapital(gamma::Float64,
      ...
      println("computing undistorted version...")
      ...
      zz = collect(range(zmin, stop=zmax, length=II));   # productivity vector
      dz = (zmax - zmin)/(II-1);
      dz2 = dz*dz;
      ...
      uu = zeros(II);
      if reflective_boundary == false
        println("natural boundary condition")
        A = spdiagm( 0 => ones(II),
                    1 => ones(II-1),
                    -1 => ones(II-1),
                    2 => ones(II-2),
        ...
        println("reflective boundary condition")
        A = spdiagm( 0 => ones(II),
                    1 => ones(II-1),
                    -1 => ones(II-1));
      end
      ...
      for n=1:maxit
          ...
          if logadjustment == true
            dstar_onecapital_log!(d_F, zz, II, rho, V, model, preloadd, CF, foc, clowerlim);
            dstar_onecapital_log!(d_B, zz, II, rho, V, model, preloadd, CB, foc, clowerlim);
          else
            dstar_onecapital_quadratic!(d_F, zz, II, rho, V, model, preloadd, CF, foc, clowerlim);
            dstar_onecapital_quadratic!(d_B, zz, II, rho, V, model, preloadd, CB, foc, clowerlim);
          end
          cF = copy(CF)
          cB = copy(CB)
    
          # Drifts
          drifts!(mu_1_F, mu_z_F, d_F, s1_F, s2_F, zz, II, model, logadjustment);
          drifts!(mu_1_B, mu_z_B, d_B, s1_B, s2_B, zz, II, model, logadjustment);
          ...
          robust_control_variables!(s1, s2, lambda, zz, V, Vz, II, model);
          robust_control_variables!(s1_F, s2_F, lambda_F, zz, V, Vz_F, II, model);
          robust_control_variables!(s1_B, s2_B, lambda_B, zz, V, Vz_B, II, model);
          ...
          if reflective_boundary == false
            ...
            create_Aval_reflective!(Aval, d_, c_, e_, II)
          end
          A.nzval .= Aval;
          ...
          println("dF max = ",maximum(d_F));
          println("dB max = ",maximum(d_B));
          println("hk max = ",maximum(hk));
          println("hk min = ",minimum(hk));
          println("hz max = ",maximum(hz));
          println("hz min = ",minimum(hz));
          println("lambdaF max = ",maximum(lambda_F));
          println("lambdaF min = ",minimum(lambda_F));
          println("lambdaB max = ",maximum(lambda_B));
          println("lambdaB min = ",minimum(lambda_B));
          println("----------------------------------")
    ...
    val = v[II_half];     # Value at (r_0, z_0)  (objective)
...
function stationary_distribution(A::SparseMatrixCSC{Float64,Int64},
      ...
      z = range(zmin, stop=zmax, length=I)';   # productivity vector
      dz = (zmax - zmin)/(I-1);
